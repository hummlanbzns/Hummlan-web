...
import { Bug, ShoppingBag, Filter } from 'lucide-react';
      ...
      <header className="bg-white border-b sticky top-0 z-10 shadow-sm">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-8">
            <Link href="/" className="text-gray-900 font-medium hover:text-green-600">Home</Link>
            <Link href="/about" className="text-gray-600 font-medium hover:text-green-600">Our Standards</Link>
          </nav>
        </div>
      </header>
            ...
            {products.map((product: any) => (
              <Link key={product.id} href={`/product/${product.slug}`} className="group bg-white border rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col h-full">
                <div className="aspect-square bg-gray-50 flex items-center justify-center relative">
                  <ShoppingBag
                    className="w-16 h-16 text-gray-200 group-hover:scale-110 group-hover:text-green-100 transition-all duration-500"
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <p className="text-xs text-green-600 font-bold uppercase tracking-wider mb-2">{product.brand_name}</p>
                  <h3 className="font-bold text-gray-900 mb-4 group-hover:text-green-700 transition-colors leading-snug flex-grow">{product.name}</h3>
