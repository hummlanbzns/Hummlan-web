...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const reviewSchema = ratings.length > 0
    ? ratings.map((r) => {
        ...
        const description = r.description || `Sustainability rating for ${pillarName}`;
        return {
          '@type': 'Review',
          author: {
            '@type': 'Organization',
            ...
            itemListElement: [
              {
                ...
                item: {
                  '@type': 'Thing',
                  name: `${pillarName} score: ${score}/${maxScore}`,
                },
              },
            ],
          },
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
            ...
            <div className="aspect-square bg-white rounded-2xl flex items-center justify-center border shadow-sm overflow-hidden group">
              <ShoppingBag className="w-32 h-32 text-gray-200 group-hover:scale-110 transition-transform duration-500" aria-label={`Product image for ${product.name}`} />
            </div>
